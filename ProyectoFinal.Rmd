...
& \hspace{0.3cm}\text{propias del colegio.} \\
...
Sean $I = \lbrace 1,3,6\rbrace$, $J = \lbrace 2,4\rbrace$ y $\mathcal{H} = \lbrace 1, 2,3\rbrace$, de ahora en adelante se asumirá $\ i \in I, \ j \in J, h \in \mathcal{H}$ para aliviar la notación, entonces:
...
Así pues, con un límite de error de estimación dado por $B=0.8878$, se tiene que el tiempo promedio semanal estimado que los estudiantes invierten en estudiar independientemente, está dado por $\hat{\mu}_2=8.3243$, y con un 95% de confianza $\mu_2 \in (7.4365,9.212)$
...
De esta manera, con un límite de error de estimación dado por $B=0.6251$ se tiene que el tiempo promedio semanal estimado que los estudiantes invierten en leer de manera proactiva, está dado por $\hat{\mu}_4=4.2943$, y con un 95% de confianza $\mu_4 \in (3.6692,4.9193)$
...
De allí, con una confianza del 95% y un límite de error estimación de 4.54%, $p_6 = 0.3677$ donde su respectivo intervalo de confianza está dado por (0.3224, 0.4131)
...
En otras palabras, se estima que el número total de estudiantes que consideran útil el modelo de alternancia es de 38, con un límite de error de estimación estimado de 5 estudiantes y con una confianza del 95%, $A_6 \in (33 , 42)$
...
es significativamente menor que la proporción que de 
