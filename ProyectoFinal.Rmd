...
cuya varianza está dada por $\widehat{VAR}(\hat{\mu}_j) = \sum_{h} W_{h}^{2} \widehat{VAR}(\overline{\rm y}_{j}^{(h)})$, donde $\widehat{VAR}(\overline{\rm y}_{j}^{(h)}) = \left(1 - \frac{n_h}{N_h}\right)\frac{S_{j}^{2(h)}}{n_h}$.
...
result1 <- estP(nh, ph.1, Nh = Nh, Wh = Wh)$info #resultados pregunta1
result1A <- estA(nh, ph.1, Nh = Nh)$info %>% round  #resultados pregunta1 A
...
result2 <- estMu(nh, muh.2, sh2.2, Nh, Wh)$info #resultados pregunta2
...
result3 <- estP(nh, ph.3, Nh = Nh, Wh = Wh)$info #resultados pregunta3
result3A <- estA(nh, ph.3, Nh = Nh)$info %>% round  #resultados pregunta3 A
...
result4 <- estMu(nh, muh.4, sh2.4, Nh, Wh)$info #resultados pregunta4
...
result6 <- estP(nh, ph.6, Nh = Nh, Wh = Wh)$info #resultados pregunta6
result6A <- estA(nh, ph.6, Nh = Nh)$info %>% round  #resultados pregunta6 A
```
...
rownames(results) <- paste("Pregunta", c(1:4, 6), sep = "")
```
